  ...
  LOOP
    ...
    IF acc = 'update' THEN
      IF v_dni IS NULL THEN
        RAISE EXCEPTION 'Fila %: no se puede actualizar sin DNI', idx;
      END IF;
      SELECT id INTO v_id FROM public.concurrentes WHERE dni = v_dni LIMIT 1;
      IF v_id IS NULL THEN
        RAISE EXCEPTION 'Fila %: no existe concurrente con DNI %', idx, v_dni;
      END IF;
      UPDATE public.concurrentes c SET
        ...
        mutual = COALESCE(NULLIF(d->>'mutual',''), c.mutual),
        ...
        dias_especificos = COALESCE(d->>'dias_especificos', c.dias_especificos),
        ...
        observaciones_administrativas = COALESCE(NULLIF(d->>'observaciones_administrativas',''), c.observaciones_administrativas),
    ...
    ELSE
      INSERT INTO public.concurrentes (
        nombre, apellido, dni, fecha_nacimiento, obra_social, mutual, n_afiliado, prestacion,
        responsable, telefono, wsp, mail, direccion, lugar_firma, dias_x_semana,
        dias_especificos, horarios, transporte, observaciones, observaciones_administrativas, tipo, activo
      ) VALUES (
        ...
        COALESCE(d->>'dias_especificos',''),
        COALESCE(d->>'horarios',''), COALESCE((d->>'transporte')::boolean, false),
        COALESCE(d->>'observaciones',''), COALESCE(d->>'observaciones_administrativas',''),
        COALESCE(NULLIF(d->>'tipo',''),'prestacion'), true
      );
      n_ins := n_ins + 1;
    END IF;
  END LOOP;
